...
#
% if node.metadata.get('atlassian-bamboo', {}).get('jvm_args', False):
JVM_SUPPORT_RECOMMENDED_ARGS="${node.metadata.get('atlassian-bamboo', {}).get('jvm_args')}"
% else:
JVM_SUPPORT_RECOMMENDED_ARGS="-XX:+UseG1GC"
% endif
% if node.metadata.get('atlassian-bamboo', {}).get('enable_jmx', False):
JVM_SUPPORT_RECOMMENDED_ARGS="-Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.port=5002 -Dcom.sun.management.jmxremote.authenticate=false -Dcom.sun.management.jmxremote.ssl=false <%text>${JVM_SUPPORT_RECOMMENDED_ARGS}</%text>"
% endif
...
#
JVM_MINIMUM_MEMORY="${node.metadata.get('atlassian-bamboo', {}).get('jvm_min_mamory', "1024m")}"
JVM_MAXIMUM_MEMORY="${node.metadata.get('atlassian-bamboo', {}).get('jvm_max_mamory', "1024m")}"
<%text>
#
